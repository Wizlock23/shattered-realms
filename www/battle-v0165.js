...
/* Shattered Realms v0.16.5 — Battle HUD synchronizer */
(()=>{
  ...
  const BATTLE_BACKGROUNDS=[
    {id:'aurora-spires',url:'../assets/ui/battle-bg-v0165-1.jpg',position:'center 26%'},
    {id:'shardfall-heights',url:'../assets/ui/battle-bg-v0165-2.jpg',position:'center 34%'},
    {id:'citadel-watch',url:'../assets/ui/battle-bg-v0165-3.jpg',position:'center 30%'},
    {id:'riverfall-throne',url:'../assets/ui/battle-bg-v0165-4.jpg',position:'center 28%'}
  ];
  function chooseBattleBackground(){
    try{
      const last=localStorage.getItem('sr_battle_bg_last');
      const pool=BATTLE_BACKGROUNDS.filter(bg=>bg.id!==last);
      const pick=(pool.length?pool:BATTLE_BACKGROUNDS)[Math.floor(Math.random()*(pool.length?pool:BATTLE_BACKGROUNDS.length))] || BATTLE_BACKGROUNDS[0];
      localStorage.setItem('sr_battle_bg_last',pick.id);
      return pick;
    }catch{return BATTLE_BACKGROUNDS[Math.floor(Math.random()*BATTLE_BACKGROUNDS.length)] || BATTLE_BACKGROUNDS[0];}
  }
  function applyBattleBackground(forceNew=false){
    if(!applyBattleBackground.current || forceNew) applyBattleBackground.current=chooseBattleBackground();
    const pick=applyBattleBackground.current;
    if(!pick) return;
    document.documentElement.style.setProperty('--battle-bg-image',`url('${pick.url}')`);
    document.documentElement.style.setProperty('--battle-bg-position',pick.position||'center center');
    document.body.dataset.battleBg=pick.id;
  }
  function syncHud(){
  ...
  window.addEventListener('load',()=>{applyBattleBackground(true);syncViewport();forceHostFullscreen();setTimeout(forceHostFullscreen,60);setTimeout(forceHostFullscreen,250);
    ...
    $('startBtn')?.addEventListener('click',()=>applyBattleBackground(true),true);
    $('resetBtn')?.addEventListener('click',()=>applyBattleBackground(true),true);
