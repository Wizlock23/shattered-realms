/* Shattered Realms v0.14.1 — compact reference battle board */
(()=>{
  ...
  function header(){
    let h=document.querySelector('.sr141-battle-header');if(h)return h;
    h=document.createElement('div');h.className='sr141-battle-header';
    h.innerHTML=`<button id="sr141BattleBack" type="button" aria-label="Back">${I.back}</button><div class="title"><span class="crossed">⚔</span><span>Battle</span></div><button id="sr141BattleGear" type="button" aria-label="Battle guide">${I.gear}</button>`;
    $('game')?.prepend(h);
    h.querySelector('#sr141BattleBack').onclick=()=>{const target=(location.origin&&location.origin!=='null')?location.origin:'*';try{window.parent?.postMessage({type:'sr-close-battle'},target)}catch{}};
    h.querySelector('#sr141BattleGear').onclick=()=>guide().classList.toggle('open');
    return h;
  }
  function guide(){
    let g=document.querySelector('.sr141-battle-guide');if(g)return g;
    g=document.createElement('div');g.className='sr141-battle-guide';
  ...
  function dock(){
    let d=document.querySelector('.sr141-battle-dock');if(d)return d;
    d=document.createElement('div');d.className='sr141-battle-dock';
    d.innerHTML=`<div class="sr141-energy-orb"><span id="sr141EnergyNow">0</span><small>/<span id="sr141EnergyMax">0</span></small></div><div id="sr141EndSlot"></div><button class="sr141-dock-btn" id="sr141LogBtn" type="button" aria-label="Battle log">${I.menu}</button>`;
    document.body.appendChild(d);
    const end=$('endTurnBtn');if(end)d.querySelector('#sr141EndSlot').appendChild(end);
    d.querySelector('#sr141LogBtn').onclick=()=>{guide().classList.remove('open');document.body.classList.toggle('sr141-log-open')};
    return d;
  }
  function makeStack(panel,id,label){
    let box=panel?.querySelector('.sr141-card-stack');if(box)return box;
    box=document.createElement('div');box.className='sr141-card-stack';box.id=id;box.setAttribute('aria-label',label);
  ...
  function syncCounters(){
    ...
    const es=document.querySelector('#enemyLeaderPanel .sr141-card-stack');if(es){es.querySelector('b').textContent=eh;es.title=`Enemy hand: ${eh} • Deck: ${ed}`}
    const ps=document.querySelector('#playerLeaderPanel .sr141-card-stack');if(ps){ps.querySelector('b').textContent=pd;ps.title=`Your deck: ${pd}`}
    const energy=$('playerEnergy')?.textContent?.match(/(\d+)\/(\d+)/);if(energy){$('sr141EnergyNow').textContent=energy[1];$('sr141EnergyMax').textContent=energy[2]}
  ...
  function arrange(){
    ...
    document.body.classList.add('sr141-mobile-battle');
    header();guide();dock();
    ...
    makeStack(enemy,'sr141EnemyStack','Enemy hand');makeStack(player,'sr141PlayerStack','Your deck');
  ...
  function observe(){
    ...
    const target=$('targetPanel');if(target)new MutationObserver(()=>document.body.classList.toggle('sr141-targeting',!target.classList.contains('hidden'))).observe(target,{attributes:true,attributeFilter:['class']});
  }
